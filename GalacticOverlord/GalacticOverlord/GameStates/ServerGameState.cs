...
namespace GalacticOverlord.GameStates
{
    ...
    {
        public ServerGameState(GalacticOverlordGame galacticOverlordGame, Channel channel, string remotePlayerId)
        ...
        public override void Initialize()
        {
            ...
            m_humanPlayer = new HumanPlayer(Color.YellowGreen, m_channel);

            m_netServerPlayer = new NetServerPlayer(m_channel, m_remotePlayerId);
            m_netServerPlayer.RemoteDisconnect += OnRemoteDisconnect;

            MapGenerator.GenerateDuelMap(m_space, m_humanPlayer, m_netServerPlayer);
        ...
        private void OnTappedDisconnect(object sender, EventArgs eventArgs)
        {
            if (m_channel.IsOpen)
            {
                m_netServerPlayer.RequestDisconnect();
                Thread.Sleep(1000);
                m_channel.Close();
            }
        ...
        private void OnRemoteDisconnect(object sender, EventArgs eventArgs)
        {
            ...
            m_screenOverlay.ClearText();
            m_screenOverlay.AddText(m_headerFont, new Vector2(240, 320), Color.Crimson,
                m_remotePlayerId);
        ...
        private NetServerPlayer m_netServerPlayer;
